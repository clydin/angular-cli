...
export class PackageManager {
  private readonly manifestCache = new Map<string, PackageManifest | null>();
  private readonly metadataCache = new Map<string, PackageMetadata | null>();
   ...
   * @param options.dryRun If true, no commands will be executed, but they will be logged.
   * @param options.logger A logger instance for debugging and dry run output.
   */
  constructor(
    private readonly host: Host,
    private readonly cwd: string,
    private readonly descriptor: PackageManagerDescriptor,
    private readonly options: { dryRun?: boolean; logger?: Logger } = {},
  ) {
  ...
  async add(
    ...
    const args: readonly string[] = [this.descriptor.addCommand, packageName, ...flags];
   ...
   */
  async install(
    options: { timeout?: number; force?: boolean; registry?: string } = {},
  ): Promise<void> {
    const flags = [options.force ? this.descriptor.forceFlag : ''].filter((flag) => flag);
  ...
  /**
   * Gets the installed version of a package from the project's dependencies.
   * @param packageName The name of the package to check.
   * @param options Extra options for the command.
   * @returns A promise that resolves to the installed version string, or `null` if the package is not installed.
   */
  async getInstalledPackageVersion(
    packageName: string,
    options: { registry?: string } = {},
  ): Promise<string | null> {
    const args: readonly string[] = [...this.descriptor.getInstalledVersionCommand, packageName];

    return this.#fetchAndParse(
      args,
      (stdout, logger) =>
        this.descriptor.outputParsers.getInstalledVersion(stdout, packageName, logger),
      options,
    );
  }
