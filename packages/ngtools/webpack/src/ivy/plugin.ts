...
export class AngularWebpackPlugin {
  ...
  private webpackSources?: Compiler['webpack']['sources'];
  ...
  private createFileEmitter(
    ...
    return async (file: string) => {
      ...
      // If sourcemap was generated by TypeScript, check if an intermediate sourcemap is present
      // and if so merge the two into one map. An intermediate sourcemap will exist if sourcemaps
      // are enabled and substitutions were performed on the input file.
      if (content && map) {
        const intermediateMap = this.sourceFileCache?.getFileSourcemap(filePath);
        if (intermediateMap) {
          assert.ok(this.webpackSources, 'File emitter is used prior to Webpack compilation');

          const finalMap = new this.webpackSources.SourceMapSource(
            content,
            sourceFile.fileName,
            map,
            undefined,
