...
async function executeInliner(
  ...
  try {
    for (const localeOption of locales) {
      await inliner.inlineForLocale(localeOption.locale, localeOption.translation);
    }
  } finally {
    await inliner.close();
  }
...
export function createStandardAppScenario(options: ScenarioFactoryOptions = {}): BenchmarkScenario {
  ...
  return {
    ...
    async run() {
      ...
      await executeInliner(workload.files, workload.locales, options.concurrency);
    },
  };
}
...
export async function primeCache(cacheDir: string, concurrency?: number): Promise<void> {
  ...
  const locales = generateTranslations(DEFAULT_LOCALES_8, 1000);
  await executeInliner(files, locales, concurrency, cacheDir);
}
...
export function createPersistentCacheWarmScenario(
  ...
  return {
    ...
    async run() {
      ...
      await executeInliner(workload.files, workload.locales, options.concurrency, cacheDir);
    },
    async teardown() {
      if (cacheDir) {
